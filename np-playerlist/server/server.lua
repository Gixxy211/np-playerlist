...
RegisterNetEvent('np-playerlist:server:manualUpdate')
AddEventHandler('np-playerlist:server:manualUpdate', function()
    local src = source
    if permission(src) then
        TriggerClientEvent('np-playerlist:client:manualUpdate', src, activePlayers,disconnectedPlayers)
    else
    end
end)
